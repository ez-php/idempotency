...
```
src/
  IdempotencyMiddleware.php   — MiddlewareInterface: stores/replays responses per Idempotency-Key
tests/
  TestCase.php                — base test case
  IdempotencyMiddlewareTest.php — replay, 409/422/400, 5xx, streamed, TTL, scope (ArrayDriver + ArrayLock)
```
...
**`IdempotencyMiddleware`** — the whole module. For POST/PATCH/PUT/DELETE requests carrying an
`Idempotency-Key` header it runs the handler once and replays the stored response afterwards.

| Situation | Result |
|---|---|
| No header, or safe method (GET/HEAD/OPTIONS) | passed through |
| Key not matching `[A-Za-z0-9_.:-]{1,255}` | 400 |
| Same key, different method/URI/body | 422 |
| Same key while the first request is still running (lock held) | 409 |
| Same key, first request completed | stored status/headers/body + `Idempotent-Replayed: true` |

Constructor: `CacheInterface $cache`, `int $ttl = 86400`, `int $lockTtl = 30`, `?Closure $scope = null`.

---

## Design Decisions and Constraints

- **Own module, not part of `rate-limiter`.** Different concern (replay vs. throttling); the core and `ez-php/http` stay untouched.
- **Soft dependency on `ez-php/cache`**: `require-dev` + `suggest` only, same pattern as `http-client`. The middleware type-hints `CacheInterface`, so it is unusable without a cache package installed.
- **Fingerprint** = sha256 of `METHOD \n URI \n sha256(rawBody)`. URI includes the query string.
...
- No external services: tests use `EzPhp\Cache\ArrayDriver` and `ArrayLock` (reset in `setUp`). No Docker services beyond the app container.
- Test classes live in the shared `Tests\` namespace but must be uniquely named
  ...
  `Idempotency` when the obvious name is already taken.
- `run()` is final on PHPUnit's `TestCase` — don't name helpers `run`.
...
|---|---|
| Cache/lock drivers (Redis, File, Array) | `ez-php/cache` |
| Request throttling | `ez-php/rate-limiter` |
| Request/response value objects, streamed responses | `ez-php/http` |
| Authentication / deciding the `$scope` value | `ez-php/auth` or application code |
| Database-backed idempotency storage | not provided; supply a `CacheInterface` implementation |
